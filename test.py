...
img = cv2.imread("data_test/test3.jpg")
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

# frontal_face
box_frontal,w_frontal = fu.detect(gray,detect_frontal_face)
...
# left_face
box_left, w_left = fu.detect(gray,detect_perfil_face)
if len(box_left)==0:
    box_left = []
    name_left = []
else:
...
box_right, w_right = fu.detect(gray_flipped,detect_perfil_face)
if len(box_right)==0:
    box_right = []
    name_right = []
else:
